# Logic and Proof

Topics 4--7 of INF1003 cover propositional and predicate logic, rules of
inference, and methods of proof. Topics 4--6 appear in Quiz 2 (Week 9, 15%),
while Topic 7 appears in Quiz 3 (Week 12, 15%).

## Topics

| Topic | Title | Key concepts |
|---|---|---|
| 4 | Propositional Logic | Propositions, connectives, converse/contrapositive/inverse, precedence, De Morgan's laws, logical equivalence, tautologies/contradictions/contingencies, satisfiability |
| 5 | Predicate Logic | Predicates, universal and existential quantifiers, negating quantified expressions, nested quantifiers |
| 6 | Rules of Inference | Modus ponens/tollens, syllogisms, addition, conjunction, simplification, propositional resolution |
| 7 | Proof Methods | Direct proof, contraposition, trivial/vacuous proof, contradiction, counterexamples, exhaustive proof, proof by cases, existence and uniqueness proofs |

## Lectures

- [Topic 4: Propositional Logic](01%20Lectures/Topic%204%20-%20Propositional%20Logic.md)
- [Topic 5: Predicate Logic](01%20Lectures/Topic%205%20-%20Predicate%20Logic.md)
- [Topic 6: Rules of Inference](01%20Lectures/Topic%206%20-%20Rules%20of%20Inference.md)
- [Topic 7: Proof Methods](01%20Lectures/Topic%207%20-%20Proof%20Methods.md)

Each lecture note consolidates the official topic slides, companion slides, and
Rosen chapter material.

## Tutorials

- [Tutorial 4: Propositional Logic](02%20Tutorials/Tutorial%204%20-%20Propositional%20Logic.md)
- [Tutorial 5: Predicate Logic](02%20Tutorials/Tutorial%205%20-%20Predicate%20Logic.md)
- [Tutorial 6: Rules of Inference](02%20Tutorials/Tutorial%206%20-%20Rules%20of%20Inference.md)
- [Tutorial 7: Proof Methods](02%20Tutorials/Tutorial%207%20-%20Proof%20Methods.md)

Each tutorial guide summarises the skills tested, suggested solution approaches,
and common pitfalls.

## Revision Notes

- [Logic and Proof Summary](03%20Revision%20Notes/Logic%20and%20Proof%20Summary.md)

Consolidated revision covering propositional logic, predicate logic, rules of
inference, and proof methods.

## Reference Guides

- [Propositional Logic Reference](04%20Reference%20Guides/Propositional%20Logic.md)
- [Predicate Logic Reference](04%20Reference%20Guides/Predicate%20Logic.md)
- [Rules of Inference Reference](04%20Reference%20Guides/Rules%20of%20Inference.md)
- [Proof Methods Reference](04%20Reference%20Guides/Proof%20Methods.md)

Quick-reference sheets with key definitions, logical equivalences, inference
rules, and proof strategies.
